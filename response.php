...
if(isset($_POST['uname'],$_POST['pwd'])){
	...
	if($uname == 'admin' && $pwd == 'admin'){
		...
		setcookie('user_cookie',$session_id,time()+60*60*24*365,'/');
		setcookie('csrf_cookie',$_SESSION['csrf'],time()+60*60*24*365,'/');
...
<html>
    ...
    var getcookies = decodedCookie.split(';');
    for(var i = 0; i <getcookies.length; i++) {
        var cookie = getcookies[i];
